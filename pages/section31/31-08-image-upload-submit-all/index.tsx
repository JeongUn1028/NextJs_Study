...
export default function ImageUploadPage(): JSX.Element {
  ...
  const onChangeFile = async (
    ...
    //* 3.Image Validation, isValid가 true면 업로드, false면 중단
  ...
  const onClickSubmit = async (): Promise<void> => {
    ...
    //* 2.이미지 업로드 하고 URL 받아오기
    const resultFiles = await Promise.all(
      files.map(
        async (
          file: File
        ): Promise<FetchResult<Pick<IMutation, "uploadFile">>> =>
          await uploadFile({ variables: { file } })
      )
    );
    const imageUrls = resultFiles.map(
      (result): string => result.data?.uploadFile.url ?? ""
    );

    //* 3.나머지 데이터와 함께 게시물 등록하기
